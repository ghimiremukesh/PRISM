...
class VLLMClient:
    ...
    # def to_ns(self, value):
    #     if isinstance(value, dict):
    #         return SimpleNamespace(**{k: self.to_ns(v) for k, v in value.items()})
    #     if isinstance(value, list):
    #         return [self.to_ns(v) for v in value]
    #     return value
    ...
    def generate(
        ...
        if response.status_code == 200:
            if generation_kwargs:
                # outputs = response.json()["prm_outputs"]
                return response.json()["prm_outputs"]
            else:
