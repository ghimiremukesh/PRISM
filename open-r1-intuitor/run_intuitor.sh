...
nohup env CUDA_VISIBLE_DEVICES=0 trl vllm-serve --model "HuggingFaceTB/SmolLM3-3B" > vllm-serve-int-smol-dapo.log 2>&1 &
VLLM_PID=$!
...
nohup env CUDA_VISIBLE_DEVICES=1,2,3,4,5,6,7 ACCELERATE_LOG_LEVEL=info\
    ...
    src/open_r1/intuitor.py --config recipes/SmolLM3-3B/intuitor/config_demo.yaml --wandb_project open-r1 --run_name SmolLM-INTUITOR-DAPO > run_intuitor-smol-dapo.log 2>&1 &
TRAINING_PID=$!
