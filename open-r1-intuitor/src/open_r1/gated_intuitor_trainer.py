...
class GatedINTUITORTrainer(Trainer):
    """
    Trainer for the Group Relative Policy Optimization (GRPO) method. This algorithm was initially proposed in the
    paper [DeepSeekMath: Pushing the Limits of Mathematical Reasoning in Open Language Models](https://huggingface.co/papers/2402.03300).

    Addition: Add periodic supervision stage, during which the advantages are gated with true accuracy. 
    ...
    Args:
        ...
        ## additions ##
        supervision_interval: after how many interval to perform the supervision
        supervision_window: how long to perform the supervision
    """
    ...
    def __init__(
        ...
        # additions
        supervision_interval: int = 2,
        supervision_window: int = 2,
    ):
        ############################################ additions ###############################

        self.sup_interval = supervision_interval
        self.sup_window = supervision_window
        self.cycle_len = self.sup_interval + self.sup_window
    ...
    ############################### addition #################################
    def _in_supervision_phase(self, step: int) -> bool:
        if self.cycle_len == 0: # treat always as unsupervised
            return False
        return (step % self.cycle_len) >= self.sup_interval
        # return True ## try always supervision for debug
    ##########################################################################
    ...
    def _generate_and_score_completions(
        ...
        rewards = (rewards_per_func * self.reward_weights.to(device).unsqueeze(0)).nansum(dim=1)  # original code
        
        ################################### add the supervision here ########################################
        """
        logic: 1. check if its time to supervise
               2. change 0/1 reward to (-1, 1) reward
        """
        if self._in_supervision_phase(self.state.global_step):
            gate_rewards =  rewards_per_func.nansum(dim=1) # assuming there's just one reward; instead of (-1, 1) do 0, 1
            gate_rewards = gate_rewards.view(-1)
        else:
            gate_rewards = torch.tensor(1., dtype=torch.float32).to(device) # regular code
        #####################################################################################################
        ...
        ################################### addition ###############################################
        sce_advantage_mean = sce_advantage_mean * gate_rewards ## change to multiplication since we want to scale | also move it up
        ############################################################################################
        ...
        # sce_advantage = sce_advantage + advantages  # original code
        ...
        ######################### addition : log the gate_reward for debugging ############
        self._metrics[mode]["rewards/supervision"].append(self._in_supervision_phase(self.state.global_step))  # just add the max to log supervision or not. 
        # wandb.log({"supervision": self._in_supervision_phase(self.state.global_step)}, step=self.state.global_step)
        ##################################################################################
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        mode = "train" if self.model.training else "eval"
        metrics = {key: sum(val) / len(val) if key != "rewards/supervision" else float(val[-1]) for key, val in self._metrics[mode].items()}  # average the metrics
