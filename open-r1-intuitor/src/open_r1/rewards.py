...
def accuracy_reward(completions: list[list[dict[str, str]]], solution=None, answer=None, **kwargs) -> list[Optional[float]]:
    """Reward function that checks if the completion is the same as the ground truth."""
    contents = [completion[0]["content"] for completion in completions]
    solution = solution if solution is not None else answer

    if solution is None:
        raise ValueError("Either 'solution' or 'answer' must be on the dataset column.")
...
def noise_reward(
    ...
    **kwargs
    ...
    """Reward function that assigns pure random binary noise (0.0 or 1.0) to each completion."""
    # One reward per example in `completions`
    rewards: list[Optional[float]] = []
    for _ in completions:
        # random.randint(0, 1) yields either 0 or 1
        rewards.append(float(random.randint(0, 1)))
    return rewards
