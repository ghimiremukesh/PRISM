...
class INTUITORTrainer(Trainer):
    ...
    mc_samples: int = 6,
        ...
        """
        Per-token Self-BALD score (epistemic mutual information) computed with
        MC-dropout.  Output tensor has the same shape as the original self-certainty
        routine:  (B, logits_to_keep).

        Higher MI  ⇒  larger *disagreement* across weight samples.
        If your reward convention is “higher = more certainty”, remember to negate
        or subtract this value when you build the final reward.

        Args
        ----
        model            HF CausalLM.  Will be toggled to .train() **only inside**
                        this function to activate dropout, then restored.
        input_ids        (B, L)  prompt + generated tokens.
        attention_mask   (B, L)
        logits_to_keep   how many trailing tokens to score (same semantics as before)
        batch_size       micro-batch size for memory budgeting; default = whole batch.
        mc_samples       number of dropout passes (K in BALD).

        Returns
        -------
        mi_token         (B, logits_to_keep)  mutual-information per token.
        """

        B_tot = input_ids.size(0)
        batch_size = batch_size or B_tot
        device = input_ids.device
        out_chunks = []

        was_training = model.training
        model.train()                     # turn dropout ON for reward pass

        for s in range(0, B_tot, batch_size):
            e = s + batch_size
            ids  = input_ids[s:e]
            mask = attention_mask[s:e]

            # running accumulators
            ent_each_sum = torch.zeros(
                ids.size(0), logits_to_keep, device=device, dtype=torch.float32
            )
            prob_sum = torch.zeros(
                ids.size(0), logits_to_keep, model.config.vocab_size,
                device=device, dtype=torch.float16  # fp16 saves 2× RAM
            )

            with torch.no_grad():
                for _ in range(mc_samples):
                    logits = model(ids, attention_mask=mask).logits  # (Bµ, L, V)
                    logits = logits[:, -logits_to_keep:, :]          # (Bµ, L_keep, V)

                    logp   = F.log_softmax(logits, dim=-1)           # fp32 for stability
                    # p = exp(logp)  (cast to fp16 to save memory)
                    p      = torch.exp(logp).to(torch.float16)

                    ent_each = -(p.to(torch.float32) * logp).sum(-1) # (Bµ, L_keep)
                    ent_each_sum += ent_each                        # accumulate ⟨H⟩

                    prob_sum += p                                    # accumulate ⟨p⟩

            # finish mixture entropy
            p_mix  = (prob_sum / mc_samples).to(torch.float32)       # back to fp32
            ent_mix = -(p_mix * p_mix.clamp_min(1e-20).log()).sum(-1)

            mi_tok = ent_mix - (ent_each_sum / mc_samples)           # (Bµ, L_keep)
            out_chunks.append(mi_tok)

            # free GPU memory early
            del ent_each_sum, prob_sum, p_mix, ent_mix, mi_tok
            torch.cuda.empty_cache()

        model.train() if was_training else model.eval()
        
        return torch.cat(out_chunks, dim=0)         # (B, logits_to_keep)
    ...
    def _generate_and_score_completions(
        ...
        with torch.no_grad():
            ...
            sce_advantage_raw = self._get_per_token_self_bald(
                self.model, prompt_completion_ids, attention_mask, logits_to_keep, batch_size
            )
        ...
        self._metrics[mode]["sb_advantage"].append(sce_advantage_record)
        self._metrics[mode]["sb_advantage_std"].append(sce_advantage_std_record)
