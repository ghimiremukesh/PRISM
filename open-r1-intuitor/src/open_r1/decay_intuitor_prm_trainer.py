...
# import ipdb
from GenPRM.genprm_batch_inference_w_completeness import GenPRM
...
def extract_user_prompt(templated_text, model="qwen"):
    import re
    
    if "qwen" in model.lower() or "smol" in model.lower():
        # Pattern to match content between <|im_start|>user and <|im_end|>
        pattern = r'<\|im_start\|>user\n(.*?)<\|im_end\|>'
    elif "olmo" in model.lower():
        pattern = r'(?s)<\|user\|\>\s*(.*?)\s*<\|assistant\|\>'
    elif "llama" in model.lower():
        pattern = r'<\|start_header_id\|>user<\|end_header_id\|>\n\n(.*?)<\|eot_id\|>'
    else:
        raise ValueError(f"Unsupported model type: {model}. Supported models are 'qwen', 'llama', and 'olmo'. Add support for your model if needed.")
    
    match = re.search(pattern, templated_text, re.DOTALL)
...
class INTUITORTrainer(Trainer):
    ...
    def __init__(
        ...
        self.include_intuitor = args.include_intuitor
        self.code_problem = args.code_problem
        
        self.model_family = model if isinstance(model, str) else model.config._name_or_path
        self.model_family = self.model_family.split("/")[-1]  # Get the last part of the model name

        # scheduler for 
        self.sc_weight = 1.0 # initial sc weight
        self.last_mean_length = None  # last step's mean of the terminated length; init as None
        ...
        if isinstance(model, str):
            ...
            model = AutoModelForCausalLM.from_pretrained(model, trust_remote_code=True, **model_init_kwargs) # trust remote code for some models. 
        else:
        ...
        self._textual_logs = {
            ...
            "judgements": deque(maxlen=maxlen),
            "prompts_to_prm": deque(maxlen=maxlen),
            "prm_rewards": deque(maxlen=maxlen),
        }
        ...
        if self.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                if args.prm_server_host is None:
                    args.prm_server_host = args.vllm_server_host

                self.prm_client = VLLMClient(args.prm_server_host, 8081, connection_timeout=args.vllm_server_timeout)
                # self.prm_client.init_communicator()
        ...
        ## initialize GenPRM model only on the main accelerator 
        if self.accelerator.is_main_process:
            self.prm_model = GenPRM(self.prm_client, args.prm_model)

        ## how about we init the GenPRM only on the main accelerator? 
        # if self.accelerator.is_main_process:
        #     self.prm_model = GenPRM('GenPRM/GenPRM-1.5B', tensor_parallel_size=1)
    ...
    def _generate_and_score_completions(
        ...
        ## get prm rewards but only on the main process
        # first gather all
        completions_text = gather_object(completions_text)

        # empty for other processes
        # ipdb.set_trace()
        if self.accelerator.is_main_process:
            # use all_prompts_text
            # prm_rewards = torch.zeros(len(all_prompts_text), device=device)
            prm_prompts = []
            for p in all_prompts_text:
                user_prompt = extract_user_prompt(p, model=self.model_family) # change for different models
                # ipdb.set_trace()
                if self.code_problem:
                    prm_prompts.append([
                    {"role": "system", "content": "You are a teacher. Your task is to review, critique the python code step by step, and check if the code is complete. Pay attention that you should not solve the problem."},
            ...
            # don't split just get the rewards. 
            # drafts = [draft.split('\n\n') for draft in completions_text]
            for c, d in zip(prm_prompts, completions_text):
                # for para in d:
                    # c.append({"role": "user", "content": para})
                    # c.append({"role": "assistant", "content": ''})
                # just add the assistant in the end. 
                # if "llama" in self.model_family.lower():
                #     c.append({"role": "assistant", "content": ""})  # add empty because it requires alternate user/asistant chat
                c.append({"role": "user", "content": d})
                c.append({"role": "assistant", "content": ''})
                
            # ipdb.set_trace()
            if self.code_problem:
                prm_outputs, prm_rewards = self.prm_model.inference(prm_prompts, verify=False, execute=False, logging=False, 
                                                                    analyze_template="<analyze>\nLet's analyze the codes step by step:", 
                                                                    output_template="<output>Let's provide judgements for each of the major steps.\nStep 1 **Judgement**: $\\boxed")
            else:
                prm_outputs, prm_rewards = self.prm_model.inference(prm_prompts, verify=False, execute=False, logging=False)
                
            prm_rewards = torch.tensor(prm_rewards, device=device).view(-1)
            # with tqdm(total=len(prm_prompts)) as pbar:
            #     for (idx, sol) in enumerate(prm_prompts):
            #         curr_reward = []
            #         curr_out = []
            ...
            #                 continue
            #             output, r = self.prm_model.inference(sol[:i], cur_step=int(i/2), verify=False, execute=False, logging=False)
            #             # ipdb.set_trace()
            #             sol[i]['content'] = output[0]
            #             curr_reward.append(r)
            #             curr_out.append(output[0])
                    
            # prm_outputs.append(curr_out)
            # prm_rewards[idx] = torch.tensor(sum(curr_reward)/max(len(curr_reward), 1), device=device)

            # pbar.update(1)
        else:
            prm_rewards = torch.empty(len(all_prompts_text), device=device)
            prm_outputs = []
            prm_prompts = []
        ...
        # prm_rewards = gather(prm_rewards) # already gathered
        # Broadcast the completions from the main process to all processes, ensuring each process receives its
        # corresponding slice.
        prm_rewards = broadcast(prm_rewards, from_process=0)
        ...
        ## compute grouped-wise prm rewards
        # ipdb.set_trace()
        mean_prm_rewards = prm_rewards.view(-1, self.num_generations).mean(dim=1)
        std_prm_rewards = prm_rewards.view(-1, self.num_generations).std(dim=1)

        mean_prm_rewards = mean_prm_rewards.repeat_interleave(self.num_generations, dim=0)
        std_prm_rewards = std_prm_rewards.repeat_interleave(self.num_generations, dim=0)
        prm_advantages = prm_rewards - mean_prm_rewards
        prm_advantages = prm_advantages / (std_prm_rewards + 1e-4)
        ...
        if self.include_intuitor:
            ...
            # weight sce with prm rewards
            sce_advantage_mean = sce_advantage_mean #* mean_prm_rewards.view(-1, self.num_generations)
            #############################################################################################
        ...
        self._metrics[mode]["prm_advantage"].append(mean_prm_rewards.mean().item())
        self._metrics[mode]["prm_advantage_std"].append(std_prm_rewards.mean().item())
        if self.include_intuitor:
        ...
        self._metrics[mode]["sc_weight"].append(self.sc_weight)
        # Log prompt and completion texts
        self._textual_logs["prompt"].extend(gather_object(prompts_text))
        self._textual_logs["completion"].extend(gather_object(completions_text))
        self._textual_logs["judgements"].extend(gather_object(prm_outputs))
        self._textual_logs["prompts_to_prm"].extend(gather_object(prm_prompts))
        self._textual_logs["prm_rewards"].extend(prm_rewards.tolist())
        for i, name in enumerate(self.reward_func_names):
            self._textual_logs["rewards"][name].extend(rewards_per_func[:, i].tolist())

        
        ## monitor mean_terminated_length
        # get running average of the terminated length
        # if self.last_mean_length is None:
        #     self.last_mean_length = term_completion_mask.float().mean().item()
        # else:
        #     # check if it is greater than 10% of the last mean_terminated_length
        #     if agg_completion_mask.float().mean().item() > 1.1 * self.last_mean_length:
        #         # reduce the sc weight by a half
        #         self.sc_weight *= 0.5
        #     else:
        #         self.last_mean_length = term_completion_mask.float().mean().item()
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        # filter to numeric values and give MLflow a step counter
        # numeric_logs = {k: float(v) for k, v in logs.items()
        #         if isinstance(v, (int, float))}
        
        # if self.accelerator.is_main_process:
        #     if mlflow.active_run() is None:       # start the run once, not every step
        #         mlflow.start_run(run_name="prm_intuitor_training")

        #     mlflow.log_metrics(numeric_logs, step=self.state.global_step)   
        ...
        if self.accelerator.is_main_process and self.log_completions:
            ...
            # if self.args.report_to and "mlflow" in self.args.report_to and mlflow.active_run() is not None:
                import pandas as pd
                ...
                table = {
                    ...
                    "prompts_to_prm": self._textual_logs["prompts_to_prm"],
                    "judgements": self._textual_logs["judgements"],
                    "prm_rewards": self._textual_logs["prm_rewards"],
                    **self._textual_logs["rewards"],
                }
                df = pd.DataFrame(table)
                # make judgements as text
                import html
                df["judgements"] = df["judgements"].apply(lambda x: html.escape(str(x)))
                df["prompts_to_prm"] = df["prompts_to_prm"].apply(lambda x: html.escape(str(x)))
                
                if self.wandb_log_unique_prompts:
                    df = df.drop_duplicates(subset=["prompt"])
                wandb.log({"completions": wandb.Table(dataframe=df)})
                # mlflow.log_table(data=df, artifact_file="completions.json", index=False)
