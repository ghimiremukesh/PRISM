...
class INTUITORTrainer(Trainer):
    ...
    @profiling_decorator
    def _get_per_token_entropy(
        self, model, input_ids, attention_mask, logits_to_keep, batch_size=None
    ) -> torch.Tensor:
        """
        Computes per-token entropy in batches.
        """
        batch_size = batch_size or input_ids.size(0)
        all_entropy = []
        # Process inputs in chunks
        for i in range(0, input_ids.size(0), batch_size):
            ...
            logits = logits[:, -logits_to_keep:, :]    # keep only the generated positions
            logits = logits / self.temperature         # follow your temperature convention

            # Efficient entropy:  H = log Σ e^{l_i} − Σ p_i · l_i
            log_Z   = torch.logsumexp(logits, dim=-1)              # (B, L_keep)
            probs   = torch.nn.functional.softmax(logits, dim=-1)                    # (B, L_keep, V)
            exp_L   = (probs * logits).sum(dim=-1)                 # (B, L_keep)
            ent     = log_Z - exp_L                                # (B, L_keep)

            all_entropy.append(ent)
        
        return torch.cat(all_entropy, dim=0)  # (B, L_keep)
    ...
    def _generate_and_score_completions(
        ...
        with torch.no_grad():
            ...
            sce_advantage_raw = -self._get_per_token_entropy(
                self.model, prompt_completion_ids, attention_mask, logits_to_keep, batch_size
            )  # -ve because we want to minimize entropy
