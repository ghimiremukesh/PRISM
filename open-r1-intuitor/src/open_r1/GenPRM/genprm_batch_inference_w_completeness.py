...
class GenPRM:
    ...
    def get_completion_score(self, out):
        '''calculate the reward score'''
        generated_text = out.text
        logprobs = out.logprobs
        tokens = out.token_ids
    ...
    def inference(
        ...
        comp_template = "<output>Let's provide judgement if the provided solution is complete irrespective of the correctness of the solution.\n**Complete**: $\\boxed",
        logging=True
    ):
        '''
...
#         return results
    ...
    def _batch_inference(
        ...
        verify_template="<verify>\nLet's use python code to find any potential error:\n```python\n",
        output_template="<output>\n**Judgement**: $\\boxed",
        comp_template = "<output>Let's provide judgement if the user's solution is complete, i.e., does the solution provide an answer to the question (irrespective of correctness)?.\n**Judgement**: $\\boxed",
        logging=True
    ):
        """
        Batch inference for multiple message inputs.
        ...
        verify_start = verify_template.format(**context)
        output_start = output_template.format(**context)
        comp_start = comp_template.format(**context)
        ...
        for i, output_node in enumerate(out_nodes):
            if output_node is not None:
                ...
                # Should not happen, but handle gracefully
                # results.append(("Error: No output generated", 0.5))
                reward_scores.append(0.5)

        # now get completion score
        # prepare prompts for completeness check
        comp_prompts = [prompts[i] + out_nodes[i].text + comp_start for i in range(batch_size)]
        sampling_params_kwargs = {
            "n": 1, 
            "temperature": TEMPERATURE,
            "top_p": TOP_P,
        ...
        comp_outputs_raw = self.model.generate(comp_prompts, generation_kwargs=sampling_params_kwargs)
        comp_outputs = [SimpleNamespace(**output) for output in comp_outputs_raw]


        # import ipdb
        # ipdb.set_trace()
        # final_out_node = [None] * batch_size
        # process outputs
        for i, output in enumerate(comp_outputs):
            # output.text = comp_prompts[i] + output.text

            # get reward score for completeness
            comp_reward_score = self.get_completion_score(output)
            completion_scores.append(comp_reward_score)

            # output.text = comp_prompts[i] + output.text
            output.text = out_nodes[i].text + comp_start + output.text # no need to store all prompts, just the PRM's outputs

            # final_out_node[i] = output
            if output is not None:
                results.append((output.text, (2 * reward_scores[i] * comp_reward_score) / (reward_scores[i] + comp_reward_score)))
            else:
                # Should not happen, but handle gracefully
                results.append(("Error: No output generated", (2 * 0.5 * reward_scores[i]) / (0.5 + reward_scores[i])))
