...
def _rfind_sublist(haystack: List[int], needle: List[int]) -> Optional[int]:
    """Return index of *last* token of the last occurrence of `needle`."""
    n = len(needle)
    for i in range(len(haystack) - n, -1, -1):
        if haystack[i : i + n] == needle:
            return i + n - 1          # index of the needle’s last token
    return None

class GenPRM:
    def __init__(self, vllm_client, model_path, tensor_parallel_size=4):
        # Load the model and tokenizer
        # timestamped_print(f"Loading model {model_path}", level="INFO")
        # self.model = LLM(
        #     model=model_path,
    ...
    def _single_inference(
        ...
        if analyze:
            ...
            output1 = self.model.generate([prompt + analyze_start], generation_kwargs=sampling_params_kwargs)[0]
            # ipdb.set_trace()
            output1 = SimpleNamespace(**output1) # for downsteam tasks
            # ipdb.set_trace()
            if verify:
        ...
        while len(cur_prompts) > 0:
            ...
            if left_tokens > 0 and cur_time < time_limit:
                ...
                output2 = self.model.generate([prompt + cur_prompts[0]], generation_kwargs=sampling_params_kwargs)[0]
                output2 = SimpleNamespace(**output2) # for downsteam tasks)
            else:
