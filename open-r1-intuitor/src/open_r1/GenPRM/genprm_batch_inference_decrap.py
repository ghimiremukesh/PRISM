    ...
    def get_reward_score(self, out):
        ...
        if boxed_match:
            ...
            if decision == "Yes":
                ...
                if yes_token_str in token_logprobs[target_token_index]:
                    yes_logprob = token_logprobs[target_token_index][yes_token_str]['logprob']
                    yes_prob = math.exp(yes_logprob)
                else:
                    # If Yes token not in logprobs, use the actual token's probability
                    actual_token_str = str(tokens[target_token_index])
                    if actual_token_str in token_logprobs[target_token_index]:
                        yes_logprob = token_logprobs[target_token_index][actual_token_str]['logprob']
                        yes_prob = math.exp(yes_logprob)
                    else:
                        # Fallback to neutral
                ...
                if no_token_str in token_logprobs[target_token_index]:
                    no_logprob = token_logprobs[target_token_index][no_token_str]['logprob']
                    no_prob = math.exp(no_logprob)
                else:
                ...
                if no_token_str in token_logprobs[target_token_index]:
                    ...
                    actual_token_str = str(tokens[target_token_index])
                    if actual_token_str in token_logprobs[target_token_index]:
                        no_logprob = token_logprobs[target_token_index][actual_token_str]['logprob']
                        no_prob = math.exp(no_logprob)
                    else:
                        # Fallback to neutral
    ...
    logging=True
        ...
        while len(active_prompts) > 0 and cur_time < time_limit:
            ...
            if len(batch_prompts_to_generate) > 0:
                ...
                outputs2 = self.model.generate(prompt + cur_prompts[0], sampling_params, use_tqdm=False)[0].outputs[0]
            ...
            for idx, (orig_idx, cur_prompt) in enumerate(zip(active_indices, active_prompts)):
                if orig_idx not in [x[1] for x in prompt_to_index_map]:
                    ...
                    forced_output  = self.model.generate(prompt + cur_prompts[0], sampling_params, use_tqdm=False)[0].outputs[0]
