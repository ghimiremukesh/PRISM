...
nohup env CUDA_VISIBLE_DEVICES=0 trl vllm-serve --model "internlm/internlm2_5-1_8b-chat" > vllm-serve-intuitor-prm.log 2>&1 &
VLLM_PID=$!
PIDS+=($VLLM_PID)
echo "vLLM server started with PID: $VLLM_PID"

# Monitor first vLLM server in background
monitor_process $VLLM_PID "vLLM server student" &
MONITOR_PID=$!
PIDS+=($MONITOR_PID)

# Run GenPRM server in the background with nohup
nohup env CUDA_VISIBLE_DEVICES=1 trl vllm-serve --model "GenPRM/GenPRM-7B" --port 8081 > vllm-serve-intuitor-prm_teacher.log 2>&1 &
GENPRM_PID=$!
PIDS+=($GENPRM_PID)
...
nohup env CUDA_VISIBLE_DEVICES=2,3,4,5,6,7 ACCELERATE_LOG_LEVEL=info\
    ...
    src/open_r1/intuitor_prm.py --config recipes/InternLM-1.8B/prm/config_genprm.yaml --wandb_project open-r1 --run_name Qwen2.5-3B-PRM-COMP_300steps > run_intuitor-prm_300.log 2>&1 &
TRAINING_PID=$!
PIDS+=($TRAINING_PID)
