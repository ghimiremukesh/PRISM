...
def evaluate(
    ...
    *,
    ...
    ds = load_dataset(hf_dataset, name='main', split=split)
    ...
    crit_prompt = (
        ...
        "YOU MUST END WITH EITHER 'The answer is correct' OR 'The answer is wrong'."
    )

    # crit_prompt = (
    #     "Check the math solution step-by-step. Generate 3 test cases to check the solution. If all test cases pass, say 'the answer is correct' else say 'the answer is wrong'."
    #     "You must provide a final response with either 'the answer is wrong' or 'the answer is correct'."
    # )
    ...
    if rank == 0 or not distributed:
        ...
        for i, (row_data, draft, final, crit, draft_prompt, rev_prompt, crit_prompt) in enumerate(zip(ds, drafts, finals, critiques, draft_prompts, store_rev_prompts, critique_prompts)):
            ...
            if dataset_name == "gsm8k":
                match = re.search(r'(?m)^#{2,}\s*([\d,]+)\s*$', answer_text)
                if match:
                    answer_text = match.group(1)        # '70000'
...
def main():
    ...
    parser.add_argument("--model",      default="/fsx/ubuntu/users/mghmr/Intuitor/open-r1-intuitor/data/Qwen2.5-3B-PRM-Test_w_sc")
