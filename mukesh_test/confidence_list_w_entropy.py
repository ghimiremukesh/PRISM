"""
From: github.com/backprop07/Self-Certainty

Calculates self-certainty of the evaluation data and stores in a json file
...
def sequence_entropy(log_probs: torch.Tensor,
                     attention_mask: torch.Tensor) -> list[float]:
    """
    Compute mean entropy (in nats) for each sequence.
      log_probs     : (batch, seq_len, vocab) — *log*-softmaxed
      attention_mask: (batch, seq_len)        — 1 for real tokens, 0 for padding
    Returns:
      list[float] length == batch, entropy averaged over non-pad tokens.
    """
    # p * log p   (broadcast over vocab dim)
    probs = log_probs.exp()
    token_ent = -(probs * log_probs).sum(dim=-1)        # (batch, seq_len)

    # mask out padding and average
    masked = token_ent * attention_mask
    seq_ent = masked.sum(dim=-1) / attention_mask.sum(dim=-1)
    return seq_ent.tolist()
...
@torch.no_grad()
def confidence_with_file(filepath, output_file=None, batch_size=4, model_dir=None, input_field_name="model_input", output_field_name="output", input_field_fallback="model_input"):
    ...
    for index in tqdm.tqdm(range(processed_items, total_items), desc="Processing inputs"):
        ...
        if item[input_field_name] is None:
            input_to_pick = item[input_field_fallback]
        else:
            input_to_pick = item[input_field_name]
        ...
        final_entropies = [None] * len(outputs)
        ...
        for group_name in ["small", "medium", "large"]:
            ...
            for batch_idx in range(num_batches):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    batch_logprob = llm(batch_ids, attention_mask=batch_attention_mask).logits
                    batch_logprob = batch_logprob[:, input_length:, :]
                    batch_logprob = F.log_softmax(batch_logprob, dim=-1).to('cpu').to(torch.float32) 
                    batch_logprob_sum = batch_logprob.sum(dim=-1)
                ...
                batch_entropy_list = sequence_entropy(batch_logprob, batch_output_attention_mask)
                group_entropies.extend(batch_entropy_list)
            ...
            for i, orig_idx in enumerate(group_indices):
                ...
                final_entropies[orig_idx] = group_entropies[i]
        ...
        new_item["entropy_list"]    = final_entropies
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_field_name", type=str, default="full_prompt", help="Field name for the input text in the input file.")
    parser.add_argument("--output_field_name", type=str, default="predictions", help="Field name for the output text in the input file.")

    # additional
    parser.add_argument("--input_field_fallback", type=str, default="full_initial_prompt", help="Field to fallback to if the original field is None.")

    args = parser.parse_args()
    
    confidence_with_file(args.input_file, args.output_file, args.batch_size, args.model_name, args.input_field_name, args.output_field_name, args.input_field_fallback)